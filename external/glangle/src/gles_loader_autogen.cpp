...
GenericProc load_gles_func_windows(const char* func_name)
{
    static HMODULE hModule = LoadLibrary("libGLESv2.dll");
    if (hModule == NULL) {
        ...
        std::cout << "load library error: " << message << std::endl;
        return nullptr;
    }

    return (GenericProc)GetProcAddress(hModule, func_name);
}
#endif

ANGLE_UTIL_EXPORT void LoadUtilGLES()
{
#if defined(_WIN32)
    LoadUtilGLES(load_gles_func_windows);
#else
#error not support
#endif
}
